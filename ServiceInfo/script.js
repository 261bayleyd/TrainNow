...
(function () {
  function tryFromHash() {
    // Expect: #/SERVICE/YYYY/MM/DD  (hash routing avoids 404s on static servers)
  ...
  function tryFromQuery() {
    // Expect: ?service=U9018&date=2025/11/08
  ...
  function tryFromPath() {
    // Support /SERVICE/YYYY/MM/DD or just /SERVICE (defaults to today)
    ...
    if (parts.length >= 4) {
      ...
      // If only a service ID is given, use today's date
  ...
  window.addEventListener("hashchange", initAutofill); // react if hash changes
})();
// ------------------------------------------------------------------------
// --- Auto-fill from URL (hash, query, or path) and auto-submit -----------
(function () {
